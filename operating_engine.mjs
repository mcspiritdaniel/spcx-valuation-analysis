...
export function runValuation(evResult, VC) {
  const totalBasicShares = VC.classA + VC.classB + VC.anysphereShares + VC.echoStarShares;
...
export function runFullEngine(schema) {
  ...
  return { operating, investment, adjustments, wacc, ev, valuation };
}

// ---- Run against the live schema file, end to end ----
import { readFileSync } from 'fs';
const schema = JSON.parse(readFileSync(new URL('./spcx_control_schema.json', import.meta.url)));
const fullResult = runFullEngine(schema);
console.log(JSON.stringify(fullResult, null, 0));
